...
Harness.test({
    ...
    pg: {
        text:
            'SELECT "user".*, "user"."id" AS "foo_id", "user"."name" AS "foo_name", "user"."id" AS "bar_id", "user"."name" AS "bar_name" FROM "user"',
        string:
            'SELECT "user".*, "user"."id" AS "foo_id", "user"."name" AS "foo_name", "user"."id" AS "bar_id", "user"."name" AS "bar_name" FROM "user"'
    },
    sqlite: {
    ...
    oracle: {
        ...
        string:
            'SELECT "user".*, "user"."id" "foo_id", "user"."name" "foo_name", "user"."id" "bar_id", "user"."name" "bar_name" FROM "user"'
    },
    params: []
});
...
Harness.test({
    query: user
        .select(user.id)
        .from(user)
        .where(user.name.equals('foo').or(user.name.equals('bar'))),
    pg: {
...
Harness.test({
    query: user
        .select(user.id)
        .from(user)
        .where(user.name.equals('foo').and(user.name.equals('bar'))),
    pg: {
...
Harness.test({
    ...
    pg: {
        ...
        string:
            'SELECT "user"."id" FROM "user" WHERE ((("user"."name" = \'foo\') OR ("user"."name" = \'baz\')) AND ("user"."name" = \'bar\'))'
    },
    sqlite: {
        text: 'SELECT "user"."id" FROM "user" WHERE ((("user"."name" = $1) OR ("user"."name" = $2)) AND ("user"."name" = $3))',
        string:
...
Harness.test({
    query: user
        .select(user.id)
        .from(user)
        .where(user.name.in(['foo', 'bar']).and(user.id.equals(1))),
    pg: {
...
Harness.test({
    ...
    pg: {
        text:
            'SELECT "user"."id" FROM "user" WHERE ((("user"."name" = $1) AND ("user"."id" = $2)) OR (("user"."name" = $3) AND ("user"."id" = $4)))',
        string:
            'SELECT "user"."id" FROM "user" WHERE ((("user"."name" = \'boom\') AND ("user"."id" = 1)) OR (("user"."name" = \'bang\') AND ("user"."id" = 2)))'
    },
    sqlite: {
        text:
            'SELECT "user"."id" FROM "user" WHERE ((("user"."name" = $1) AND ("user"."id" = $2)) OR (("user"."name" = $3) AND ("user"."id" = $4)))',
    ...
    mysql: {
        text:
            'SELECT `user`.`id` FROM `user` WHERE (((`user`.`name` = ?) AND (`user`.`id` = ?)) OR ((`user`.`name` = ?) AND (`user`.`id` = ?)))',
        string:
            "SELECT `user`.`id` FROM `user` WHERE (((`user`.`name` = 'boom') AND (`user`.`id` = 1)) OR ((`user`.`name` = 'bang') AND (`user`.`id` = 2)))"
    },
    mssql: {
