...
Harness.test({
    ...
    pg: {
        text:
            "SELECT relname FROM pg_class WHERE oid IN ( SELECT indexrelid FROM pg_index, pg_class WHERE pg_class.relname='post' AND pg_class.relnamespace IN (SELECT pg_namespace.oid FROM pg_namespace WHERE nspname = 'public') AND pg_class.oid=pg_index.indrelid)",
        string:
            "SELECT relname FROM pg_class WHERE oid IN ( SELECT indexrelid FROM pg_index, pg_class WHERE pg_class.relname='post' AND pg_class.relnamespace IN (SELECT pg_namespace.oid FROM pg_namespace WHERE nspname = 'public') AND pg_class.oid=pg_index.indrelid)"
    },
    mysql: {
