...
Harness.test({
    query: post
        .alter()
        .dropColumn(post.content)
        .dropColumn(post.userId),
    pg: {
...
Harness.test({
    query: group
        .alter()
        .addColumn('id', 'varchar(100)')
        .addColumn('userId', 'varchar(100)'),
    pg: {
        text: 'ALTER TABLE "group" ADD COLUMN "id" varchar(100), ADD COLUMN "userId" varchar(100)',
        string: 'ALTER TABLE "group" ADD COLUMN "id" varchar(100), ADD COLUMN "userId" varchar(100)'
    },
...
const post2 = Table.define<{ userId: number; picture: string }>({
    name: 'post',
    columns: [
        {
            name: 'userId',
...
Harness.test({
    query: post2.alter().addColumn(post2.userId),
    pg: {
...
Harness.test({
    query: post2.alter().addColumn(post2.picture),
    pg: {
